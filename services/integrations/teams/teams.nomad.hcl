variable "image" {
  type        = string
  default     = "ghcr.io/legionio/legion-teams:latest"
  description = "Container image for legion-teams"
}

variable "count" {
  type        = number
  default     = 1
  description = "Number of Teams integration instances"
}
      ...
      template {
        data        = <<-EOF
        {
          ...
          "vault": {
            ...
            "namespace": "legionio",
            "kerberos": { "auth_path": "auth/kerberos/login" }
          },
          "api": { "enabled": false }
        }
        EOF
