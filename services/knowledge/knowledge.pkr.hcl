...
build {
  sources = ["source.docker.knowledge"]

  # install knowledge + apollo extensions
  provisioner "shell" {
    environment_vars = [
    ...
    inline = [
      "gem install --no-document ${join(" ", local.knowledge_extensions)}",
    ]
  }
